...
""===d.trim(a)&&(b=d(b).find("input"),0<b.length&&(b=b[0],"checked"in b?a=b.checked?1:0:"value"in b&&(a=b.value)));if(a===t||isNaN(a))l=0;c.push({i:n++,t:a})}}),k=1;for(;k<c.length;k++)for(var g=c.length-1;g>=k;g--){var q=c[g],r=c[g-1],s=l?parseFloat(q.t||"0")<parseFloat(r.t||"0"):q.t<r.t;h&&(s=!s);s&&(c[g-1]=q,c[g]=r)}h=a.find("tbody");for(var p in c)d(f[c[p].i]).appendTo(h);if(e)for(g=f.length-e;g<f.length;g++)d(f[g]).appendTo(h);if(b)for(f=a.find("tbody tr").removeClass(b),a=0;a<f.length;a++)1===
