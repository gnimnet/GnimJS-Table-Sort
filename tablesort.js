...
(function($){
     ...
     */
    function TableSort(selector,highlightClass){
        var objThis=this;
        var $table=$(selector);
        if($table.length!=1){
            $table.each(function(table){
                new TableSort(table,highlightClass);
            });
            return;
        }
        ...
        for(var i=0;i<$th.length;i++){
            (function(th,index){
                $(th).click(function(){
                    ...
                    _sort($table,index,desc,highlightClass);
                });
            })($th[i],i);
        }
        objThis.$dom=$table;
     ...
     * @param highlightClass row hightlight class name
     */
    function _sort($table,index,desc,highlightClass){
        var rows=[];
        var cnt=0;
        var isNum=1;
        ...
        //reset table element
        for(var r in rows){
            $($tr[rows[r].i]).appendTo($table.find('tbody'));
        }
        if(highlightClass){
